...
var digestAuthRequest = function (method, url, username, password) {
	...
	this.makeUnauthenticatedRequest = function (data) {
		...
		self.firstRequest.setRequestHeader('Access-Control-Allow-Origin', '*');
		self.firstRequest.setRequestHeader('Access-Control-Expose-Headers', 'www-authenticate');
	...
	this.makeAuthenticatedRequest = function () {
		...
		// self.authenticatedRequest.setRequestHeader('Access-Control-Allow-Origin', '*');
		...
		if (url.includes('picture')) {
			self.authenticatedRequest.responseType = 'blob'
		}
		...
		self.authenticatedRequest.onload = function () {
			...
			if (self.authenticatedRequest.status >= 200 && self.authenticatedRequest.status < 400) {
				...
				// return data
				if (url.includes('picture')) {
					self.successFn(self.authenticatedRequest.response)
				} else if (self.authenticatedRequest.responseText !== 'undefined' && self.authenticatedRequest.responseText.length > 0) {
					// If JSON, parse and return object
		...
		self.authenticatedRequest.onerror = function () {
			self.log('Error (' + self.authenticatedRequest.status + ') on authenticated request' + url);
			// console.log('Status text: ' + self.authenticatedRequest.statusText);
			// console.log('ready state: ' + self.authenticatedRequest.readyState);
			// console.log('response text: ' + self.authenticatedRequest.responseText);
			self.nonce = null;
			self.errorFn(self.authenticatedRequest.status);
		};
	...
	this.formulateResponse = function () {
		...
		// console.log('Username: ' + username + ', realm: ' + self.realm + ', pass: ' + password + ', HA1: ' + HA1);
		var HA2 = CryptoJS.MD5(method + ':' + uriWoIP).toString();
		// console.log('method: ' + method + ', uri: ' + uriWoIP + ', HA2: ' + HA2);
		var response = CryptoJS.MD5(HA1 + ':' + self.nonce + ':' + ('00000000' + self.nc).slice(-8) + ':' + self.cnonce + ':' + self.qop + ':' + HA2).toString();
		// console.log('before MD5: ' + HA1 + ':' + self.nonce + ':' + ('00000000' + self.nc).slice(-8) + ':' + self.cnonce + ':' + self.qop + ':' + HA2);
		// console.log("nc: " + self.nc  + "  - " + ('00000000' + self.nc).slice(-8));
		return response;
	}
